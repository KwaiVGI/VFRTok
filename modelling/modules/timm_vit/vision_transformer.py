...
from modelling.modules.timm_vit.rope_utils import apply_rotary_emb, apply_rotary_emb_partial
...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        if 'rope_heads' in kwargs and kwargs['rope_heads'] is not None:
            self.rope_heads = kwargs['rope_heads']
            print(f'Attention set rope_heads={self.rope_heads}, only first {self.rope_heads} heads use RoPE')
        else:
            self.rope_heads = self.num_heads
    ...
    def forward(self, x: torch.Tensor, 
        ...
        if freqs_cis is not None:
            ...
            q_, k_ = q.clone(), k.clone()
            if num_latent_tokens == 0:
                q_[:, :self.rope_heads, num_prefix_tokens:], k_[:, :self.rope_heads, num_prefix_tokens:] = \
                    apply_rotary_emb_partial(q[:, :self.rope_heads, num_prefix_tokens:], k[:, :self.rope_heads, num_prefix_tokens:], freqs_cis=freqs_cis)
            else:
                q_[:, :self.rope_heads, num_prefix_tokens:-num_latent_tokens], k_[:, :self.rope_heads, num_prefix_tokens:-num_latent_tokens] = \
                    apply_rotary_emb_partial(q[:, :self.rope_heads, num_prefix_tokens:-num_latent_tokens], k[:, :self.rope_heads, num_prefix_tokens:-num_latent_tokens], freqs_cis=freqs_cis)
        else:
            q_, k_ = q, k
        ...
        if self.fused_attn:
            ...
            qkv = torch.stack((q_, k_, v), dim=0)
            qkv = qkv.permute(1, 3, 0, 2, 4).contiguous()
            x, attn, _ = flash_attn_qkvpacked_func(qkv, softmax_scale=self.scale, return_attn_probs=True)
            x = x.view(B, N, C)
            # attn.shape: B num_heads N
        else:
            q_ = q_ * self.scale
        ...
        x = self.proj(x)
        x = self.proj_drop(x)
        return x, attn # return attn_map for analyze
...
class MoVQBlockv2(nn.Module):
    ...
    def forward(self, x: torch.Tensor, interpolate_zq: torch.Tensor,
        ...
        attn_out, _ = self.attn(attn_out, attn_mask, freqs_cis, num_prefix_tokens, num_latent_tokens)
...
class Block(nn.Module):
    ...
    def forward(self, x: torch.Tensor, attn_mask: torch.Tensor = None, freqs_cis=None, num_prefix_tokens=1, num_latent_tokens=32) -> torch.Tensor:
        x = x + self.drop_path1(self.ls1(self.attn(self.norm1(x), attn_mask, freqs_cis, num_prefix_tokens, num_latent_tokens)[0]))
...
class ResPostBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        x = x + self.drop_path1(self.norm1(self.attn(x)[0]))
...
class ParallelThingsBlock(nn.Module):
    ...
    def _forward_jit(self, x: torch.Tensor) -> torch.Tensor:
        x = x + torch.stack([attn(x)[0] for attn in self.attns]).sum(dim=0)
        x = x + torch.stack([ffn(x) for ffn in self.ffns]).sum(dim=0)
        return x

    @torch.jit.ignore
    def _forward(self, x: torch.Tensor) -> torch.Tensor:
        x = x + sum(attn(x)[0] for attn in self.attns)
...
@register_model
def vit_tinytiny_patch14_dinov2_movq(pretrained: bool = False, **kwargs) -> VisionTransformer:
    model_args = dict(patch_size=14, embed_dim=192, depth=3, num_heads=3, init_values=1e-5, block_fn=MoVQBlockv2)
    model = _create_vision_transformer(
        'vit_tinytiny_patch14_dinov2', pretrained=pretrained, **dict(model_args, **kwargs))
    return model
