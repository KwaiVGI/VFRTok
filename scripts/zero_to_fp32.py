...
def get_latest_ckpt(folder, exp_name):
    ...
    ckpts = [x for x in ckpts if 'model' in x]
...
if __name__ == "__main__":
    ...
    parser.add_argument("-i", "--ckpt_idx", type=str, default=None)
    parser.add_argument("-t", "--tag", type=int, default=None)
    ...
    if args.ckpt_path:
        ...
        out_dir = args.ckpt_path
        tag = os.path.basename(args.ckpt_path)
    else:
        exp_name = get_exp_name(args.exp_folder, args.ckpt_idx)
        assert exp_name, f'cannot find {args.ckpt_idx} in {args.exp_folder}'
        ckpt_dir = os.path.join(args.exp_folder, exp_name, 'checkpoints')
        out_dir = os.path.join(ckpt_dir, f'{args.module}_{args.tag:07d}')
        tag = os.path.basename(out_dir)

    weights_name = "model.safetensors" if args.safe_serialization else "pytorch_model.bin"
    print(f'save to: {os.path.join(out_dir, weights_name)}')

    convert_zero_checkpoint_to_fp32_state_dict(ckpt_dir, out_dir,
